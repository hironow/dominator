...
| **How** | `dominator mcp` serves MCP tools (`get_nfr`, `get_insights`, `record_result`, `dmail`); the `/nfr-judge` skill drives mcp-k6 from the claude-code session and records the verdict through `record_result` |
...
- `get_insights` reads the learning loop: a live judgment summary derived from EventJudgmentRecorded plus the legacy hue/coefficient ledgers (read-only; refs issue 0034).
