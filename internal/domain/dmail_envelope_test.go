...
func TestParseDMailEnvelope_FixtureSchemaIsHonored(t *testing.T) {
	...
	// dominator-side fixture exercises the amadeus -> dominator
	// direction (nfr_check_request kind) so the symmetric envelope
	// contract is verified.
	path := fixturePath(t, "dmail-2026-06-01T13-00-00Z-jkl012.yaml")
	...
	// then: every Phase 2c required field is set, nullable markers stay nil.
	want := domain.DMailEnvelope{
		MessageID:      "dmail-2026-06-01T13-00-00Z-jkl012",
		SourceTool:     "amadeus",
		TargetTool:     "dominator",
		Kind:           "nfr_check_request",
		BodyPath:       "./dmail-2026-06-01T13-00-00Z-jkl012.body.md",
		CreatedAt:      time.Date(2026, 6, 1, 13, 0, 0, 0, time.UTC),
		IdempotencyKey: "sha256:b1c2d3e4f5a6071829314253647586979a0b1c2d3e4f5a6071829314253647586",
	}
...
idempotency_key: sha256:x`,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !envelopeErrContains(err.Error(), tc.want) {
...
func TestParseDMailEnvelope_DetectsConsumedEnvelope(t *testing.T) {
	// given: amadeus -> dominator envelope with ack_at set (= post-consume).
...
body_path: ./b.md
created_at: 2026-06-01T13:00:00Z
seen_at: 2026-06-01T13:05:00Z
ack_at: 2026-06-01T13:06:00Z
idempotency_key: sha256:x`
...
func TestParseDMailEnvelope_BodyPathPointsAtPairedFile(t *testing.T) {
	...
	yamlPath := fixturePath(t, "dmail-2026-06-01T13-00-00Z-jkl012.yaml")
...
// envelopeErrContains is a tiny strings.Contains alias used by the
// envelope tests. The package already has its own `contains` helper
// in config_test.go for the config tests, so this one is namespaced
// to avoid the duplicate declaration.
func envelopeErrContains(s, sub string) bool {
