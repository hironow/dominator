// Package domain rival_contract.go: Rival Contract v1 parser.
//
...
type RivalContractMetadata struct {
	Schema      string
	ID          string
	Revision    int
	Supersedes  string
...
// originating D-Mail name. Consumers use this as the projection result.
...
// EvidenceItemsToNfrConfig maps the four supported `nfr.*` evidence keys
// onto the existing dominator NfrConfig struct. Unknown keys (including
// unknown `nfr.*` keys) and non-NFR keys are silently ignored. Values
// that fail to parse as numbers are also silently dropped because the
// Evidence parser already enforces the bullet shape; only well-formed
// numeric values reach this stage in practice.
//
// Mapping:
//   - nfr.p95_latency_ms        -> Performance.P95LatencyMs    (int)
//   - nfr.error_rate_percent    -> Performance.ErrorRatePercent (float64)
//   - nfr.success_rate_percent  -> Reliability.SuccessRatePercent (float64)
//   - nfr.target_rps            -> Scalability.TargetRPS        (int)
//
// This helper is pure: it does not validate the resulting NfrConfig
// against ValidateConfig, and it does not apply DefaultConfig values.
// Callers are expected to merge the result with their own defaults.
func EvidenceItemsToNfrConfig(items []EvidenceItem) NfrConfig {
	var cfg NfrConfig
	for _, item := range items {
		switch item.Key {
		case "nfr.p95_latency_ms":
			if v, err := strconv.Atoi(item.Value); err == nil {
				cfg.Performance.P95LatencyMs = v
			}
		case "nfr.error_rate_percent":
			if v, err := strconv.ParseFloat(item.Value, 64); err == nil {
				cfg.Performance.ErrorRatePercent = v
			}
		case "nfr.success_rate_percent":
			if v, err := strconv.ParseFloat(item.Value, 64); err == nil {
				cfg.Reliability.SuccessRatePercent = v
			}
		case "nfr.target_rps":
			if v, err := strconv.Atoi(item.Value); err == nil {
				cfg.Scalability.TargetRPS = v
			}
		}
	}
	return cfg
