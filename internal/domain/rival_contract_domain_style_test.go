...
//
// dom is parser-only for this phase: the NFR path consumes the Evidence
// section and never looks at the Domain section, so no consumer change is
// required. The parser still accepts the new optional `domain_style` enum
// for parity with sj canonical (copy-sync discipline) so future consumer
// extensions can rely on a uniform metadata surface.
//
...
func TestParseRivalContractMetadata_DomainStyleOmitted_NoInference(t *testing.T) {
	...
	// that MUST NOT influence the parser.
	meta := map[string]string{
		"contract_schema":   "rival-contract-v1",
		"contract_id":       "auth-x",
		"contract_revision": "1",
