...
func TestParseRivalContractMetadata_MultiplexKeysIgnored(t *testing.T) {
	...
	if parsed.Schema != domain.SchemaRivalContractV1 {
		t.Errorf("Schema: got %q, want %q", parsed.Schema, domain.SchemaRivalContractV1)
	}
	...
	if parsed.DomainStyle != domain.DomainStyleEventSourced {
		t.Errorf("DomainStyle: got %q, want %q", parsed.DomainStyle, domain.DomainStyleEventSourced)
	}
