...
// impl; this file is a symmetric copy adapted for dominator.
//
...
func RecordMCPInvocation(ctx context.Context, toolName, status string, duration time.Duration) {
	...
	counter, err := Meter.Int64Counter("mcp.tool.invocations",
		metric.WithDescription("Total MCP tools/call invocations on dominator MCP server"),
	)
	if err == nil {
	...
	histogram, err := Meter.Float64Histogram("mcp.tool.duration",
		metric.WithDescription("Duration (seconds) of MCP tools/call invocations on dominator MCP server"),
		metric.WithUnit("s"),
	)
	if err == nil {
