...
func IsProcessAlive(pid int) bool {
	...
	cmd := exec.Command("tasklist", "/FI", "PID eq "+strconv.Itoa(pid), "/NH") // nosemgrep: go.lang.security.audit.dangerous-exec-command.dangerous-exec-command -- PID is an integer, not user-controlled string [permanent]
	out, err := cmd.Output()
	if err != nil {
		return false
	}
