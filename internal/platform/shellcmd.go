...
func NewShellCmd(ctx context.Context, cmdLine string, args ...string) *exec.Cmd {
	...
	cmd := exec.CommandContext(ctx, bin, allArgs...) // nosemgrep: go.lang.security.audit.dangerous-exec-command.dangerous-exec-command -- cmdLine comes from trusted configuration, not user input [permanent]
