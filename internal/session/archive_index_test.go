...
func TestExtractMeta_DmailWithFrontmatter(t *testing.T) {
	...
	entry := session.ExtractMeta(f, stateDir, "dominator")
	...
	if entry.Tool != "dominator" {
		t.Errorf("tool: got %q, want %q", entry.Tool, "dominator")
	}
...
func TestExtractMeta_InsightFile(t *testing.T) {
	...
	entry := session.ExtractMeta(f, stateDir, "dominator")
	if entry.Operation != "judgment" {
		t.Errorf("op: got %q, want %q", entry.Operation, "judgment")
	}
...
func TestIndexWriter_Append_CreatesFile(t *testing.T) {
	...
	entries := []domain.IndexEntry{
		{Timestamp: "2026-03-10T14:30:00Z", Operation: "judgment", Issue: "ENG-1", Status: "success", Tool: "dominator", Path: "archive/report.md", Summary: "test"},
	}
...
func TestIndexWriter_Append_AppendsToExisting(t *testing.T) {
	...
	w := &session.IndexWriter{}
	e1 := []domain.IndexEntry{{Timestamp: "t1", Operation: "o1", Tool: "dominator", Path: "p1", Summary: "s1"}}
	e2 := []domain.IndexEntry{{Timestamp: "t2", Operation: "o2", Tool: "dominator", Path: "p2", Summary: "s2"}}
...
func TestIndexWriter_Append_ValidJSON(t *testing.T) {
	...
	w := &session.IndexWriter{}
	entries := []domain.IndexEntry{
		{Timestamp: "t1", Operation: "o1", Tool: "dominator", Path: "p1", Summary: "summary with \"quotes\""},
		{Timestamp: "t2", Operation: "o2", Tool: "dominator", Path: "p2", Summary: "s2"},
	}
...
func TestIndexWriter_Rebuild_ScansAllMd(t *testing.T) {
	...
	w := &session.IndexWriter{}
	n, err := w.Rebuild(indexPath, stateDir, "dominator")
	if err != nil {
...
func TestIndexWriter_Rebuild_OverwritesExisting(t *testing.T) {
	...
	w := &session.IndexWriter{}
	n, _ := w.Rebuild(indexPath, stateDir, "dominator")
	if n != 1 {
