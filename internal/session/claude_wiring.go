...
// session auto-discovers project skills there, so /nfr-judge works
...
// mcpServerEntry is the stdio MCP server entry shape used in
// .mcp.json (dominator has no mcp-config command; init owns the
// project-root upsert).
...
func UpsertRootMCPConfig(baseDir string) (string, error) {
	...
	entry, err := json.Marshal(mcpServerEntry{Command: "dominator", Args: []string{"mcp"}})
	if err != nil {
		return "", fmt.Errorf("marshal server entry: %w", err)
	}
	servers["dominator"] = entry
