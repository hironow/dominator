...
// refs issue 0032 / decision D5(a): `dominator init` materializes the
// entry skill into the target project's .claude/skills/ so a bare
// `claude` session auto-discovers /nfr-judge without plugin
...
func TestInstallClaudeSkills_MaterializesEntrySkill(t *testing.T) {
	...
	// then
	skillPath := filepath.Join(baseDir, ".claude", "skills", "nfr-judge", "SKILL.md")
	data, err := os.ReadFile(skillPath)
	if err != nil {
		t.Fatalf("entry skill not materialized: %v", err)
	}
	text := string(data)
	if !strings.Contains(text, "name: nfr-judge") {
...
func TestInstallClaudeSkills_IsIdempotent(t *testing.T) {
	...
	// then
	if _, err := os.Stat(filepath.Join(baseDir, ".claude", "skills", "nfr-judge", "SKILL.md")); err != nil {
...
func TestUpsertRootMCPConfig_CreatesWhenAbsent(t *testing.T) {
	...
	entry, _ := servers["dominator"].(map[string]any)
	if entry == nil || entry["command"] != "dominator" {
		t.Errorf("dominator server entry missing: %v", cfg)
	}
}
...
func TestUpsertRootMCPConfig_PreservesOtherEntriesAndKeys(t *testing.T) {
  ...
  "somethingElse": {"keep": true}
	...
	// then: k6 entry (with its extra fields), foreign key, and dominator all present
	...
	if _, ok := servers["dominator"]; !ok {
		t.Errorf("dominator entry missing after upsert: %v", servers)
	}
}
