...
func NewSQLiteOutboxStore(dbPath, archiveDir, outboxDir string) (*SQLiteOutboxStore, error) { // nosemgrep: domain-primitives.multiple-string-params-go -- dbPath/archiveDir/outboxDir are semantically distinct filesystem path roles; swapping them causes structural failure caught by integration tests [permanent]
...
// derived from the .pass/ directory: DB at .run/outbox.db, targets at archive/
// and outbox/.
...
func atomicWrite(targetPath string, data []byte) error {
	...
	tmp, err := os.CreateTemp(dir, ".dominator-tmp-*")
	if err != nil {
		return err
	}
	tmpPath := tmp.Name()
